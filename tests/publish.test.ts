...
describe('the archive feed file set', () => {
  // The corpus the Librarian API answers from consumes data/issues/{N}/. The
  // canonical shape is what nine years of issues already look like there.
  it('commits the canonical trio under data/issues/{N}/', () => {
    const paths = archiveInputs(doc()).map((f) => f.path);
    expect(paths).toEqual([
      'data/issues/350/archive.md',
      'data/issues/350/links.json',
      'data/issues/350/metadata.json',
    ]);
  });

  it('front matter is canonical: no layout, permalink, tags, or notice', () => {
    const md = archiveMarkdown(doc());
    expect(md.startsWith('---\nbuttondown_id:')).toBe(true);
    for (const field of ['layout:', 'permalink:', 'tags:', 'Generated by']) {
      expect(md, `${field} belongs to the materialized site page`).not.toContain(field);
    }
    expect(md).toContain(`number: 350`);
    expect(md).toContain('word_count:');
  });

  it('receives no audio of any kind', () => {
    const md = archiveMarkdown(doc(), {
      audio: { audio_url: 'https://files.thingelstad.com/x.mp3' },
    });
    expect(md).not.toContain('audio_url');
  });

  it('links.json carries the shape the archive already stores', () => {
    const links = JSON.parse(
      archiveInputs(doc()).find((f) => f.path.endsWith('links.json'))!.content,
    );
    expect(Object.keys(links)).toEqual([
      'notable_links', 'briefly_links', 'domains', 'word_count',
    ]);
  });

  it('metadata.json carries the identity fields, Buttondown id included', () => {
    const meta = JSON.parse(
      archiveInputs(doc(), { buttondownId: 'em_abc', absoluteUrl: 'https://b.test/350/' })
        .find((f) => f.path.endsWith('metadata.json'))!.content,
    );
    expect(Object.keys(meta)).toEqual([
      'number', 'subject', 'description', 'image', 'slug',
      'publish_date', 'buttondown_id', 'absolute_url',
    ]);
    expect(meta.buttondown_id).toBe('em_abc');
    expect(meta.absolute_url).toBe('https://b.test/350/');
  });

  it('the body drops the H1 the archive prints from front matter', () => {
    const body = archiveMarkdown(doc()).split('\n---\n')[1] ?? '';
